...
- [ ] **P1.3** Normalize line endings to LF + add `.gitattributes`
- [ ] **P1.4** Doc sync: scrub `impermanence-wipe` / VBox refs from CONTRIBUTING.md + readme.md
      *(defer wipe-root mechanism docs to P4.4)*

## Phase 2 — SSH

- [ ] **P2.1** Global SSH lockdown in `nixos/base.nix` (no root, no passwords)

## Phase 3 — Honest impermanence flag

- [ ] **P3.1** Flip `impermanence = false` on `_template-bare-metal`, `nixos-desktop`, `nixos-vbox`
...
- [ ] **P4.1** Re-add `profiles.impermanence.preserveDirs` option
- [ ] **P4.2** New `nixos/modules/wipe-root.nix` (initrd btrfs subvol rollback)
- [ ] **P4.3** Btrfs subvol layout in `single-disk-uefi.nix` + `single-disk-bios.nix`
- [ ] **P4.4** Wire wipe-root vs tmpfs branching into `mkProfile` + update CONTRIBUTING.md principle 3 mechanism para
- [ ] **P4.5** ⚠ Destructive: verify on real `nixos-desktop` hardware *(ask before running)*
- [ ] **P4.6** Re-enable `impermanence = true` on `nixos-desktop` + `nixos-vbox`
...
- [ ] **P5.1** Centralize identity (`userIdentity` let-binding in `flake.nix`)
- [ ] **P5.2** Extract `mkProfile` → `nixos/lib/mk-profile.nix`
- [ ] **P5.3** Extract profiles table → `nixos/profiles.nix` *(after P5.2)*
- [ ] **P5.4** Create `_schema-detect.nix`; consume in fish/network/common
- [ ] **P5.5** Rename `modules/common/` → `modules/dual/` *(after P5.4)*
- [ ] **P5.6** Minor cleanups: `toJSON` eta-wrap, git `settings`, qemu.options tokens, persistence file modes
